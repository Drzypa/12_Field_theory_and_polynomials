...
\usepackage{pmmeta}
\pmcanonicalname{Subfield}
\pmcreated{2013-03-22 14:01:45}
\pmmodified{2013-03-22 14:01:45}
\pmowner{drini}{3}
\pmmodifier{drini}{3}
\pmtitle{subfield}
\pmrecord{7}{35055}
...
%% \include{argawarga}
\begin{document}
Let $F$ be a field and $S$ a subset of $F$ such that $S$ with the inherited operations from $F$ is also a field.\, Then we say that $S$ is a {\em subfield} of $F$.

The name ``subfield'' is used also in the case that $F$ is only a skew field and its subset $S$ forms a skew field.
%%%%%
%%%%%
\end{document}
